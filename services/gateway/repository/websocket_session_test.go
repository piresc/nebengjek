...
func TestWSSessionRepository_CleanupExpiredSessions(t *testing.T) {
	_, redisClient, cleanup := setupTestRedis(t)
	defer cleanup()
	...
	// Manually delete session data for user-2 to simulate expired session
	sessionKey := "ws:session:user-2"
	redisClient.Client.Del(ctx, sessionKey)

	// Run cleanup
	err := repo.CleanupExpiredSessions(ctx)
	assert.NoError(t, err)

	// Verify user-2 was removed from connections sets
	assert.False(t, redisClient.Client.SIsMember(ctx, "ws:connections", "user-2").Val())
	assert.False(t, redisClient.Client.SIsMember(ctx, "ws:server:server-1:connections", "user-2").Val())

	// Verify other users are still connected
	assert.True(t, redisClient.Client.SIsMember(ctx, "ws:connections", "user-1").Val())
	assert.True(t, redisClient.Client.SIsMember(ctx, "ws:connections", "user-3").Val())
}

func TestWSSessionRepository_CleanupExpiredSessions_NoExpiredSessions(t *testing.T) {
	_, redisClient, cleanup := setupTestRedis(t)
	defer cleanup()
	...
	// Run cleanup (should not remove any sessions)
	err := repo.CleanupExpiredSessions(ctx)
	assert.NoError(t, err)

	// Verify all users are still connected
	assert.True(t, redisClient.Client.SIsMember(ctx, "ws:connections", "user-1").Val())
	assert.True(t, redisClient.Client.SIsMember(ctx, "ws:connections", "user-2").Val())
}

func TestWSSessionRepository_CleanupExpiredSessions_GetConnectedUsersError(t *testing.T) {
	_, redisClient, cleanup := setupTestRedis(t)
	defer cleanup()
	...
	err := repo.CleanupExpiredSessions(ctx)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "failed to get connected users for cleanup")
}
