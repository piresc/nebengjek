...
import (
	...
	"github.com/piresc/nebengjek/auth-service/proto"
...
func main() {
	...
	// Initialize NSQ Producer (for async operations only)
	...
	authServer := &AuthServer{producer: producer, secretClient: secretClient}
	...
	// Setup REST routes
	setupRESTRoutes(router, authServer)
...
func setupRoutes(router *gin.Engine, producer *nsq.Producer) {
	...
	{
		auth.POST("/register", handleRegister(producer))
		auth.POST("/login", handleLogin(producer))
	}
}
