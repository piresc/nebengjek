...
func TestLocationUpdate_TimestampHandling(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.timestamp, update.Location.Timestamp)

			if tt.isZero {
				assert.True(t, update.CreatedAt.IsZero())
				assert.True(t, update.Location.Timestamp.IsZero())
			} else {
				assert.False(t, update.CreatedAt.IsZero())
				assert.False(t, update.Location.Timestamp.IsZero())
			}
		})
	}
}
...
func TestLocationUpdate_JSONTags(t *testing.T) {
	...
	assert.Equal(t, "ride_id", getJSONTag(&LocationUpdate{}, "RideID"))
	assert.Equal(t, "driver_id", getJSONTag(&LocationUpdate{}, "DriverID"))
	assert.Equal(t, "location", getJSONTag(&LocationUpdate{}, "Location"))
	assert.Equal(t, "created_at", getJSONTag(&LocationUpdate{}, "CreatedAt"))
}
...
func TestLocationAggregate_JSONTags(t *testing.T) {
	...
	// Test that all expected fields are present
	assert.Equal(t, "ride_id", getJSONTag(&LocationAggregate{}, "RideID"))
	assert.Equal(t, "distance", getJSONTag(&LocationAggregate{}, "Distance"))
	assert.Equal(t, "latitude", getJSONTag(&LocationAggregate{}, "Latitude"))
	assert.Equal(t, "longitude", getJSONTag(&LocationAggregate{}, "Longitude"))
}