...
func TestWebSocket_JSONTags(t *testing.T) {
	...
	// WSMessage
	assert.Equal(t, "event", getJSONTag(&WSMessage{}, "Event"))
	assert.Equal(t, "data", getJSONTag(&WSMessage{}, "Data"))
	
	// WSErrorMessage
	assert.Equal(t, "code", getJSONTag(&WSErrorMessage{}, "Code"))
	assert.Equal(t, "message", getJSONTag(&WSErrorMessage{}, "Message"))
	
	// WSHeartbeatMessage
	assert.Equal(t, "timestamp", getJSONTag(&WSHeartbeatMessage{}, "Timestamp"))
	assert.Equal(t, "sequence,omitempty", getJSONTag(&WSHeartbeatMessage{}, "Sequence"))
	
	// WSConnectionHealth
	assert.Equal(t, "user_id", getJSONTag(&WSConnectionHealth{}, "UserID"))
	assert.Equal(t, "last_activity", getJSONTag(&WSConnectionHealth{}, "LastActivity"))
	assert.Equal(t, "last_ping", getJSONTag(&WSConnectionHealth{}, "LastPing"))
	assert.Equal(t, "missed_pings", getJSONTag(&WSConnectionHealth{}, "MissedPings"))
	assert.Equal(t, "is_healthy", getJSONTag(&WSConnectionHealth{}, "IsHealthy"))
	assert.Equal(t, "connected_since", getJSONTag(&WSConnectionHealth{}, "ConnectedSince"))
	
	// WSSessionData
	assert.Equal(t, "user_id", getJSONTag(&WSSessionData{}, "UserID"))
	assert.Equal(t, "role", getJSONTag(&WSSessionData{}, "Role"))
	assert.Equal(t, "server_id", getJSONTag(&WSSessionData{}, "ServerID"))
	assert.Equal(t, "connected_at", getJSONTag(&WSSessionData{}, "ConnectedAt"))
	assert.Equal(t, "last_activity", getJSONTag(&WSSessionData{}, "LastActivity"))
	assert.Equal(t, "last_ping", getJSONTag(&WSSessionData{}, "LastPing"))
	assert.Equal(t, "last_pong", getJSONTag(&WSSessionData{}, "LastPong"))
	assert.Equal(t, "missed_pings", getJSONTag(&WSSessionData{}, "MissedPings"))
	assert.Equal(t, "is_healthy", getJSONTag(&WSSessionData{}, "IsHealthy"))
}