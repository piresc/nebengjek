...
// TracedRedisClient wraps a Redis client with automatic tracing
...
// Get gets a key with automatic tracing
...
// Set sets a key with automatic tracing
...
// Delete deletes keys with automatic tracing
...
func (r *TracedRedisClient) GeoRadius(ctx context.Context, key string, longitude, latitude float64, radius float64, unit string) ([]redis.GeoLocation, error) {
	if !r.tracer.IsEnabled() {
...
func (r *TracedRedisClient) HMGet(ctx context.Context, key string, fields ...string) ([]string, error) {
	if !r.tracer.IsEnabled() {
...
// Close closes the Redis client with automatic tracing
